#!/usr/bin/env node
/**
 * Mock fix update email send utility — manual trigger only.
 *
 * NEVER auto-sends. By default it lists the verified recipients and previews
 * the message. Delivery requires the `--send` flag.
 *
 * Usage (from repo root):
 *   node --env-file=.env scripts/send-mock-fix-email.mjs                              # dry run: list + preview
 *   node --env-file=.env scripts/send-mock-fix-email.mjs --before 2026-08-08T10:00:00Z # dry run, only accounts created before the cutoff
 *   node --env-file=.env scripts/send-mock-fix-email.mjs --send                        # deliver to all verified users
 *   node --env-file=.env scripts/send-mock-fix-email.mjs --before 2026-08-08T10:00:00Z --send  # deliver to the affected cohort only
 *   node --env-file=.env scripts/send-mock-fix-email.mjs --to user@mail.com               # preview/send one address
 *   node --env-file=.env scripts/send-mock-fix-email.mjs --send --limit 5               # send to the first 5 recipients
 *
 ...
 *   logs/send-mock-fix-email-YYYYMMDD-HHMMSS.log
 */
...
import { getMockFixUpdateEmail } from '../lib/emails/mock-fix-update.ts'

const email = getMockFixUpdateEmail()
...
function parseBeforeTimestamp(raw) {
  const ts = Date.parse(raw)
  if (Number.isNaN(ts)) {
    throw new Error(`Invalid --before timestamp: "${raw}". Use an ISO 8601 value, e.g. 2026-08-08T10:00:00Z`)
  }
  return new Date(ts)
}
...
async function writeLogFile() {
  ...
  const filePath = join(LOG_DIR, `send-mock-fix-email-${stamp}.log`)
...
async function main() {
  ...
  const before = BEFORE ? parseBeforeTimestamp(BEFORE) : null

  console.log('Preparing mock fix update email:')
  console.log(`  Subject: ${email.subject}\n`)
  if (before) {
    console.log(`  Recipient cutoff: accounts created before ${before.toISOString()}\n`)
  }
  ...
  if (before) {
    recipients = recipients.filter((u) => {
      ...
      return createdAt !== null && !Number.isNaN(createdAt.getTime()) && createdAt < before
    })
  }
  ...
  for (const u of recipients) {
    console.log(`  • ${u.email}${u.user_metadata?.username ? ` (${u.user_metadata.username})` : ''}${u.created_at ? ` — created ${u.created_at}` : ''}`)
  }
  if (!recipients.length) {
  ...
  if (DRY) {
    ...
    console.log('  node --env-file=.env scripts/send-mock-fix-email.mjs --to <one email> --send')
    if (before) {
      console.log('  node --env-file=.env scripts/send-mock-fix-email.mjs --before <timestamp> --send')
    } else {
      console.log('  node --env-file=.env scripts/send-mock-fix-email.mjs --send')
    }
    return
  }
