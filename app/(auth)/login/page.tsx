...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Referral is best-effort; don't block login
    } finally {
  ...
  return (
    <>
      {confirmed && (
        <div className="flex items-start gap-2.5 rounded-xl bg-green-50 px-4 py-3 text-xs font-semibold text-green-700">
          <CheckCircle className="h-4 w-4 shrink-0 mt-0.5 text-green-500" />
          Email confirmed — log in to continue.
        </div>
      )}

      {errParam === 'verification_failed' && (
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 active:bg-gray-100 disabled:opacity-50 min-h-[44px] cursor-pointer"
      >
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm rounded-xl border border-gray-100 bg-white p-6 shadow-xs">
        ...
        <Suspense fallback={<div className="mt-6 text-sm text-gray-400">Loading...</div>}>
          <LoginForm />
        </Suspense>
