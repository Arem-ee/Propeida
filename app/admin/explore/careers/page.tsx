...
export default async function AdminExploreCareersPage() {
    ...
    <div>
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{(items ?? []).length} careers</p>
        </div>
        <Link
          href="/admin/explore/careers/new"
            ...
            {(items ?? []).map((item) => (
              <tr key={item.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">
                  <Link
                    href={`/admin/explore/careers/${item.id}/edit`}
